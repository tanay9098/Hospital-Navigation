...
class FloorMapViewState extends State<FloorMapView> {
  ...
  void resetView(BoxConstraints constraints) {
    ...
    final floorConfig =
        kFloorConfigs[navProvider.activeFloor] ?? kFloorConfigs[0]!;
  ...
  void centerOnPosition(Offset pos, BoxConstraints constraints) {
    ...
    final double followScale = max(
      currentScale,
      _fitScale >= 1.0 ? 1.5 : _fitScale * 3.0,
    );
      ...
      builder: (context, navProvider, child) {
        ...
        if (navProvider.activeRoute != null) {
          ...
          if (route.path.isNotEmpty &&
              route.path.first.floor == navProvider.activeFloor) {
            for (var node in route.path) {
              routePoints.add(
                Offset(node.x * mapDrawScale, node.y * mapDrawScale),
              );
            }

            for (final turnPathIndex in route.turnPointIndices) {
              if (turnPathIndex < 0 || turnPathIndex >= route.path.length)
                continue;
              final turnNode = route.path[turnPathIndex];
              turnPoints.add(
                Offset(turnNode.x * mapDrawScale, turnNode.y * mapDrawScale),
              );
            }
          }
        }
                ...
                child: Stack(
                  children: [
                    ...
                    Positioned.fill(
                      child: POIOverlay(
                        nodes:
                            navProvider.floorManager.activeGraph?.nodes.values
                                .toList() ??
                            [],
                        ...
                        child: Consumer2<SimulationProvider, PdrProvider>(
                          builder: (context, simProvider, pdrProvider, _) {
                            ...
                            if (pdrProvider.isPdrEnabled &&
                                pdrProvider.currentPosition != null) {
                              final snappedPos = pdrProvider.currentPosition!;
                              if (snappedPos.floor == navProvider.activeFloor) {
                                currentSimPos = Offset(
                                  snappedPos.x * mapDrawScale,
                                  snappedPos.y * mapDrawScale,
                                );
                                mapHeading = snappedPos.heading;
                              }
                            } else if (simProvider.isSimulating &&
                                simProvider.currentFloor ==
                                    navProvider.activeFloor) {
                              currentSimPos = Offset(
                                simProvider.currentX * mapDrawScale,
                                simProvider.currentY * mapDrawScale,
                              );
                              mapHeading = simProvider.currentHeading;
                            }

                            if (currentSimPos != null &&
                                _isTrackingUser &&
                                !_pendingCenter) {
                              _pendingCenter = true;
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                            ...
                            return CustomPaint(
                              ...
                              painter: PathOverlay(
                                ...
                                isSimulating:
                                    simProvider.isSimulating ||
                                    (pdrProvider.isPdrEnabled &&
                                        currentSimPos != null),
      ...
      builder: (context, settingsProvider, child) {
        return Stack(
          ...
          children: [
            Positioned.fill(
              child: CustomPaint(
                painter: POIPainter(nodes: nodes, scale: mapDrawScale),
              ),
            ),
            // Only render labels for nodes that have a label (rooms, lifts, stairs, ramps)
            ...nodes
                .where((n) => n.label != null && n.label.toString().isNotEmpty)
                .map((node) {
                  final translatedName = settingsProvider.transliterateLabel(
                    node.label.toString(),
                  );
...
class POIPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    try {
      for (var node in nodes) {
        ...
        // so both 'TOILET FEMALE' and 'toilet_female' match consistently.
        final label = (node.label ?? '').toString().toLowerCase().replaceAll(' ', '_');
  ...
  bool shouldRepaint(POIPainter oldDelegate) =>
      oldDelegate.nodes != nodes || oldDelegate.scale != scale;
}
