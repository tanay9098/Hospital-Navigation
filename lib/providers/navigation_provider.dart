...
class NavigationProvider extends ChangeNotifier {
  ...
  UserPosition processPdrUpdate(UserPosition raw) {
    ...
    if (_lastRawPos == null) {
      ...
      // Attempt to snap initial position to path if we have no state
      if (_pathTracker.currentState != null) {
        return raw.copyWith(x: _pathTracker.currentState!.x, y: _pathTracker.currentState!.y, floor: _pathTracker.currentState!.floor);
      }
      return raw;
    }

    // Calculate physical step taken since last raw update
    double dx = raw.x - _lastRawPos!.x;
    double dy = raw.y - _lastRawPos!.y;
    double stepLength = sqrt(dx * dx + dy * dy);
    _lastRawPos = raw;

    if (stepLength > 0.001) {
      // User actually stepped!
      PathTrackerState state = _pathTracker.updateWithStep(stepLength, raw.heading, raw);
      ...
      // Just a heading update (or standing still). Keep the avatar clamped to the last snapped location.
      if (_pathTracker.currentState != null) {
        return raw.copyWith(
