...
class SimulationProvider extends ChangeNotifier {
  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 16), (timer) {
      ...
      double remainingSpeed = 0.5; // pixels per tick (approx 31 px/s) — slow enough for voice instructions
      ...
      while (remainingSpeed > 0 && _currentStepIndex < route.path.length - 1) {
        ...
        if (dist <= remainingSpeed) {
          if (dx != 0 || dy != 0) _currentHeading = atan2(dx, -dy);
          _currentX = currentTarget.x;
          _currentY = currentTarget.y;
          _currentStepIndex++;
          remainingSpeed -= dist;
